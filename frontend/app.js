...
function initData() {
    ...
    if (savedContacts) {
        ...
        // Ensure all pre-existing contacts have lastConnected, role, category, and portrait images
        const timeframes = ["2 hours ago", "Yesterday", "3 days ago", "1 week ago", "2 weeks ago", "1 month ago"];
        const roles = ["Director of Ops", "Project Manager", "Creative Lead", "CEO", "Developer", "HR Partner"];
        const categories = ["Operations", "Management", "Design", "Management", "Engineering", "Operations"];
        const photos = [
        ...
        state.contacts.forEach((contact, index) => {
            if (!contact.lastConnected) {
                contact.lastConnected = timeframes[index % timeframes.length];
                needsUpdate = true;
            }
            if (!contact.role) {
                contact.role = roles[index % roles.length];
                needsUpdate = true;
            }
            if (!contact.category) {
                contact.category = categories[index % categories.length];
                needsUpdate = true;
            }
            // Upgrade blank profile urls to gorgeous portrait photos
            if (!contact.profile_image_url || contact.profile_image_url.trim() === '') {
                contact.profile_image_url = photos[index % photos.length];
                needsUpdate = true;
            }
        });
        if (needsUpdate) {
...
function saveContactsToStorage() {
    localStorage.setItem('contacthub_contacts', JSON.stringify(state.contacts));
}
...
function showView(viewName) {
    ...
    // Only scroll to top instantly if moving between separate screens (where landing view visibility changes)
    if (!isLandingActive || (viewName !== 'landing' && viewName !== 'contacts')) {
        window.scrollTo({ top: 0, behavior: 'instant' });
    }
    
    // 1. Manage Active Main Sections
    ...
    if (viewName === 'landing' || viewName === 'contacts') {
        ...
        // Show contacts screen panel, hide others
        const screens = {
        ...
        // 2. Manage Active Dashboard Screen panels
        const screens = {
            'contacts': document.getElementById('screen-contacts'),
            'add-contact': document.getElementById('screen-add-contact'),
            'edit-contact': document.getElementById('screen-edit-contact'),
    ...
    // 3. Sync Sidebar Buttons Highlight
    ...
    sideContactsBtn.classList.remove('active');
    sideAddBtn.classList.remove('active');
    sideAboutBtn.classList.remove('active');

    if (viewName === 'contacts') sideContactsBtn.classList.add('active');
    if (viewName === 'add-contact') sideAddBtn.classList.add('active');
    if (viewName === 'about') sideAboutBtn.classList.add('active');

    // 4. Scroll main workspace panel back to top
...
function renderRecentStack() {
    const recentContacts = state.contacts.filter(c => c.lastConnected);
    ...
    for (let i = 0; i < maxCards; i++) {
        ...
        card.innerHTML = `
            ...
            <div class="stack-card-footer-centered">
                <span class="stack-last-contact-badge">Last contacted ${escapeHtml(contact.lastConnected)}</span>
            </div>
        `;
...
function handleAddFormSubmit(e) {
    ...
    const photoUrl = addPhotoBase64 || photoInput.value.trim();
    ...
    if (!email) {
        ...
        setFieldError('add-email', 'add-email-error', 'Email should be in a valid format.');
        isValid = false;
    } else if (isDuplicateEmail(email)) {
        setFieldError('add-email', 'add-email-error', 'Duplicate email addresses are not allowed.');
        isValid = false;
    }
    ...
    if (!phone) {
        ...
        setFieldError('add-phone', 'add-phone-error', 'Please enter a valid phone number.');
        isValid = false;
    }
    ...
    state.contacts.unshift(newContact); // Add to beginning
    saveContactsToStorage();
    renderContactsList();
...
function handleEditFormSubmit(e) {
    ...
    const role = roleInput.value.trim();
    const category = categorySelect.value;
    const photoUrl = editPhotoBase64 || photoInput.value.trim();
    ...
    if (!email) {
        ...
        setFieldError('edit-email', 'edit-email-error', 'Email should be in a valid format.');
        isValid = false;
    } else if (isDuplicateEmail(email, id)) {
        setFieldError('edit-email', 'edit-email-error', 'Duplicate email addresses are not allowed.');
        isValid = false;
    }
    ...
    if (!phone) {
        ...
        setFieldError('edit-phone', 'edit-phone-error', 'Please enter a valid phone number.');
        isValid = false;
    }
    ...
    if (index !== -1) {
        ...
        state.contacts[index] = {
            ...
            category: category || "General",
            profile_image_url: photoUrl,
            lastConnected: currentContact.lastConnected || "Just now"
        };
        saveContactsToStorage();
        renderContactsList();
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    confirmBtn.addEventListener('click', () => {
        ...
        if (idToDelete) {
            state.contacts = state.contacts.filter(c => c.id !== idToDelete);
            saveContactsToStorage();
            renderContactsList();
        }
        closeModal();
